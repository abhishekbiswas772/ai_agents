...
class ListDirTool(Tool):
    ...
    async def execute(self, invocation : ToolInvocation) -> ToolResult:
        ...
        if not dir_path.exists() or not dir_path.is_dir():
            return ToolResult.error_result(f"Directory doesnot exits: {dir_path}")
        try:
            items = sorted(
        ...
        if not items:
            return ToolResult.success_result("Directory is empty", metadata={"path": str(dir_path), "entries": 0})
        lines = []
        for item in items:
            if item.is_dir():
