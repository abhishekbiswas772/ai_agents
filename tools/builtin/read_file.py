...
class ReadFileParams(BaseModel):
    path : str = Field(..., description='Path to the file to read (related to working directory or absolute directory)')
    offset: int = Field(1, ge=1, description="Line number to start reading from (1-based). Defaults to 1")
    limit : int | None = Field(None, ge=1, description="Maximum number of line to read. If not spefied, read entire file")
    ...
    async def execute(self, invocation: ToolInvocation) -> ToolResult:
        ...
        if file_size > self.MAX_FILE_SIZE:
            return ToolResult.error_result(
                f"File to large ({file_size / (1024 * 1024):.2f}MB).",
                f"Maximum i ({self.MAX_FILE_SIZE / (1024 * 1024):.2f}MB)"
            )
